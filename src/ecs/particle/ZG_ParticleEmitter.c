...
bool CParticleEmitter::loadFromEmitterInfo(
	...
	for(unsigned i = 0; i < list_attribs.size(); i++){
		...
		if(!found)
			print_warning_cr("Property \"%s\" don't reconized.",list_attribs[i].c_str());

	}

	for(unsigned i = 0; i < list_app_attribs.size(); i++){
		// remove spaces
		list_app_attribs[i] = CStringUtils::remove(list_app_attribs[i],' ');
		found = false;
		...
		if(!found)
			print_warning_cr("Property \"%s\" don't reconized.",list_app_attribs[i].c_str());
	}
...
)
{
	...
	if( (emitter_attribs & CParticle::ONE_PARTICLE) == CParticle::ONE_PARTICLE)
		...
		MAX_PARTICLES = ZG_MIN(ZG_MAX((int)((MAX_PARTICLES_PER_FRAME)*(FACTOR_PARTICLES_PER_FRAME)),1),MAX_TOTAL_PARTICLES);

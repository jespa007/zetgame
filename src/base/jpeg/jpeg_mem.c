...
typedef struct source_mgr source_mgr;
...
static void mgr_init_source(j_decompress_ptr cinfo) {ZG_UNUSUED_PARAM(cinfo);}
...
static void mgr_skip_input_data(j_decompress_ptr cinfo, long num_bytes) {
  source_mgr* src = (source_mgr*)cinfo->src;
...
static void mgr_term_source(j_decompress_ptr cinfo) {ZG_UNUSUED_PARAM(cinfo);}
...
static void mgr_set_source(j_decompress_ptr cinfo, const char* data, size_t len) {
  ...
  if (cinfo->src == 0) { // if this is first time;  allocate memory
	cinfo->src = (struct jpeg_source_mgr *)(*cinfo->mem->alloc_small)
	  ((j_common_ptr) cinfo, JPOOL_PERMANENT, sizeof(source_mgr));
  }
  src = (source_mgr*) cinfo->src;
  src->pub.init_source = mgr_init_source;
  src->pub.fill_input_buffer = mgr_fill_input_buffer;
  src->pub.skip_input_data = mgr_skip_input_data;
  src->pub.resync_to_restart = jpeg_resync_to_restart; // default
  src->pub.term_source = mgr_term_source;
  // fill the buffers
...
bool jpeg_decode_mem(const uint8_t *src_data_ptr, size_t src_data_len, uint8_t **dst_data_ptr, unsigned * w, unsigned *h){
	// Variables for the source jpg
		...
		mgr_set_source(&cinfo, (const char *)src_data_ptr,src_data_len);
		...
		*h=cinfo.image_height;
		...
		if(cinfo.output_components!=3){
		 ...
		 /* We may need to do some setup of our own at this point before reading
		  ...
		  dst=*dst_data_ptr=(uint8_t *)malloc((*w)*(*h)*cinfo.output_components);
...
uint8_t * jpeg_encode_mem(unsigned char *src_data, unsigned image_width, unsigned image_height, int quality, long unsigned int * dst_data_len){
	...
	//unsigned len = width*height*bpp;
	dst_data_len=0;

	JSAMPLE * image_buffer = src_data;	/* Points to large array of R,G,B-order data */
	//unsigned len = height*width*3;
	 ...
	 /* This struct Contains the JPEG compression parameters and pointers to
	  ...
	  jpeg_mem_dest(&cinfo, &dst_data, dst_data_len);
	  ...
	  //ZG_BufferByte *b=new ZG_BufferByte(dest_data,dest_data_len);
	  //free(dest_data);

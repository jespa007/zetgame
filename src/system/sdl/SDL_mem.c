...
void  MEMSDL_PrintStatus()
{
	...
	for(unsigned i = 0; i < MAX_CHECK_SURFACES; i++){
		...
		if(pti->pointer != 0){
			...
			Log_Print(pti->file,pti->line,LOG_LEVEL_WARNING,true," SDL surface not deallocated");
			n_unllocated++;
		}
	}
...
void	 __sdlmem__SDL_FreeSurface(const  char  *file,  int  line, void  *srf)
{
	...
	if(SDLMEM_dicotomic_delete((uintptr_t)srf)){
		...
		Log_Print(file,line,LOG_LEVEL_WARNING,true," pointer was not allocated by SDL");
	}

...
static
bool isComment(const char *xmlText){
	if(
				xmlText[0+0] == '<'
				&& xmlText[0+1] == '!'
				&& xmlText[0+2]=='-'
...
static
void skpComment(XmlDoc* doc,const char **xmlText){
	if(isComment(*xmlText)){
		*xmlText+=4; // !--
		while (**xmlText){
			if(**xmlText == '\n'){
				doc->currLine = *xmlText+1;
			}
			else if(
				(*xmlText)[0]=='-'
						&&

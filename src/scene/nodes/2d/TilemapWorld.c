...
int	NUM_TILES_SORROUND=20;
	...
	float CTilemapWorld::getHeight3D(){return heightransform;}
	 ...
	 void CTilemapWorld::updateChilds()
	{
		...
		if(tilemap_data!=NULL)
		{
			...
			if((tilemap_properties & CTilemapWorld::TILEMAP_INFINITE) != 0)
			{
				...
				if(isMode7())
					ztile_cam = (int)floor((float)(m_attached_scene->getCamera()->getTranslate(CScreenObject::WORLD_COORDINATES).z/tilemap_data->heightransform));
				else
					ztile_cam = (int)floor((float)(m_attached_scene->getCamera()->getTranslate(CScreenObject::WORLD_COORDINATES).y/tilemap_data->heightransform));
			...
			float z_ini = (ztile_start*(tilemap_data->heightransform));
	...
	//		subtilemap3D[0].update();
			...
			for(float z3d=z_ini, z = 0; z < NUM_TILES_SORROUND; z++, z3d+=(tilemap_data->heightransform))

...
void CSprite2dEmitter::setupAsTextBoxNode2d(uint8_t max_texts,const std::string & ttf_font, uint8_t font_size,const CColor3i & color, unsigned short properties ){
...
void CSprite2dEmitter::setupAsTextBoxNode2d(uint8_t max_texts,const std::string & font_name, uint8_t size,const CColor3i & color ){
	setupAsTextBoxNode2d( max_texts, font_name,  size,   color,0 );
}
...
void  CSprite2dEmitter::checkCollision(CSprite2d *incoming_sprite){
	...
	for(unsigned i = 0; i < max_sprites;i++){
		...
		if(spr->life > 0){
			...
			if(CPhysics2dCollider::checkCollision(spr->getTranslate(CTransformNode::WORLD_COORDINATES)
												 ,collider,sw1,sh1
												 ,*pos_2
												 ,collider_2
												 ,sw2,sh2)

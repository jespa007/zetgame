...
bool CTilemap::load(const string & m_filename){

	string file_to_load = m_working_dir + "/"+ m_filename;

	if(Object::load(file_to_load)){
		// parse object ...
		...
		if(m_parser->parseFile(file_to_load)){
			...
			if(m_mainStructTiled->m_tilesets.size()>0 && m_mainStructTiled->m_layers.size() > 0){
				...
				ImageObject *img = NEW_IMAGE(image_name);
				/*char buf[400], *str;
				...
				/*if(str != NULL){
					...
					m_image = StrUtils::Remove(m_image, '\\');
				...
				if(StrUtils::EndsWith(m_image,".gif")){
					bytes_to_convert = 4;
				}
		...
		)
		{
			...
			if(m_texture != NULL) { // ---> texture ok!
				...
				//for(int t = 0; t < subtilemap.length; t++)*/
					...
					heightransform =  ZG_ACTIVE_RENDER->height_to_projection((float)(nTilesY*tileheight));// /(float)(m_render->getWidth());

					//inc_subTileMapX3D = (float)(width3d)/tilemap[0].length;
					//inc_subTileMapY3D = (float)(heightransform)/tilemap.length;
...
void CTilemap::draw(){
	...
	drawShape(&transform_absolute,m_tilemapGeometry, m_defaultAppearance );
}*/

void CTilemap::updateSceneGraph()
{
	updateSceneGraphGeometry();

	transform_absolute.m_scale.z=1;// set z=1 to avoid buggy light...
}

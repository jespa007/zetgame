...
BufferByte * File_Read(const char *filename){
	...
	if((fp  =  fopen(filename,"rb"))  !=  NULL)
	{
		if((file_length = File_Length(filename)) != -1) {
			...
			buffer=BufferByte_New(file_length+1); // we reserve +1 for end string just in case is a text (it always works!)
			readed_elements = fread(buffer->ptr, 1, file_length, fp);

			if((long int)readed_elements != file_length) {
				Log_Warning("number elements doesn't match with length file (%s)",filename);
		...
		else  Log_Error("I can't read file \"%s\"",filename);
	}
	else  Log_Error("I can't open file \"%s\" for reading",filename);
...
bool	File_Write(const char *filename, BufferByte * buf){
	...
	if((fp  =  fopen(filename,"w"))  !=  NULL)
	{
		...
		if(wrote_elements == buf->len) {
			...
			Log_Warning("number elements wrote doesn't match with length file (%s)",filename);
		}

		fclose(fp);
	}
	else  {
		Log_Error("I can't open file \"%s\" for writting",filename);
	}
